...
# Z_1, ...., Z_100 ~ N(5, 9) дисперсия 9, соотв-но стандарт отклонение = 3
...
sjp.lm(model) # графическое представление интервалов, если довер интервалы пересекают 0, то коэф незначимый
...
# проверка гипотезы без построения вспомогательной модели, что коэф при католик=коэф при агрикал
